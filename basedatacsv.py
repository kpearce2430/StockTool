...
def CreateEntryTypes():
    ...
    headers = ["Date","Type","Security","Symbol","Description","Shares","Amount","Account","Entry Price Per Share","Remaining Shares","Sold Lots","Price Per Share","Cost" ]
    jsonTags = ['entryDate','entryType','entrySecurity','entrySymbol','entryDescription','entryShares','entryAmount', 'entryAccount', 'entryPricePerShare', 'entryRemainingShares', 'SoldLots', 'PricePerShare', 'Cost' ]
    types = ['date','text','text','text','text','text','currency','text','currency','number','text','currency','currency']
...
class Account:
    ...
    def addEntry( self, myEntry  ):
            ...
            if myEntry.type() == 'Sell' or myEntry.type() == 'Short Sell':
                ...
                if round(curentShares,4) >= round(sellShares,4) :
                    ...
                    for e in self.entries:
                        ...
                        if (e.type() == 'Reinvest Dividend' or e.type() == 'Buy' or e.type() == 'Add Shares') and e.remainingShares() > 0.00:
        ...
        if  (myEntry.type() == 'Buy' or myEntry.type() == 'Add Shares' or myEntry.type() == 'Reinvest Dividend') and self.numShares() > self.totalPending():
            self.clearPending()
